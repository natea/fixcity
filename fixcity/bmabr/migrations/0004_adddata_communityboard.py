...
def load_all_boards_pickle():
    curdir = os.path.dirname(os.path.abspath(__file__))
    sqldir = os.path.normpath(os.path.join(curdir, '..', '..', 'sql'))
    pickle_path = os.path.join(sqldir, 'all_boards.pickle')
...
class Migration:
    ...
    def forwards(self, orm):
        all_boards = load_all_boards_pickle()
        CommunityBoard = orm.CommunityBoard
        for board_data in all_boards:
            board = CommunityBoard(gid=board_data['gid'],
                                   borocd=board_data['borocd'],
                                   board=board_data['board'],
                                   boro=board_data['boro'],
                                   the_geom=board_data['the_geom'],
                                   )
            board.save()
    

    def backwards(self, orm):
        orm.CommunityBoard.objects.all().delete()
